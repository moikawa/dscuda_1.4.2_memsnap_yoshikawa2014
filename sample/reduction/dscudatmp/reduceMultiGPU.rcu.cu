...
cudaError_t dscudaBindTextureWrapper(int *moduleid, char *texname,
                                    ...
                                    size_t size = UINT_MAX)
{
  return     dscudaBindTextureWrapper(dscudaLoadModule("./dscudatmp/reduceMultiGPU.cu.ptx", Ptxdata), "tex", offset, &tex, devPtr, &desc, size);
}
...
cudaError_t dscudaBindTextureWrapper(int *moduleid, char *texname,
                                    ...
                                    size_t size = UINT_MAX)
{
  return     dscudaBindTextureWrapper(dscudaLoadModule("./dscudatmp/reduceMultiGPU.cu.ptx", Ptxdata), "tex", offset, tex, devPtr, tex.channelDesc, size);
}
...
#pragma begin reduceMultiGPU.cuh


/*
 * stub for remote call to reduceVec0.
 */
void
dscudareduceVec0(dim3 _gdim, dim3 _bdim, size_t _smemsize, cudaStream_t _stream , int n, int *g_idata, int *g_odata)
{
    int _narg = 3;
    ...
    if (dscudaRemoteCallType() == RC_REMOTECALL_TYPE_IBV) {
...
#if !RPC_ONLY
        ibvDscudaLaunchKernelWrapper(dscudaLoadModule("./dscudatmp/reduceMultiGPU.cu.ptx", Ptxdata), 0, mangledname_,
        ...
        rpcDscudaLaunchKernelWrapper(dscudaLoadModule("./dscudatmp/reduceMultiGPU.cu.ptx", Ptxdata), 0, mangledname_,
...
/*
 * stub for remote call to reduceVec3.
 */
void
dscudareduceVec3(dim3 _gdim, dim3 _bdim, size_t _smemsize, cudaStream_t _stream , int n, int *g_idata, int *g_odata)
{
    int _narg = 3;
    int _ibvgdim[3], _ibvbdim[3];
    IbvArg _ibvarg[3], *_ibvargp;
    ...
    if (dscudaRemoteCallType() == RC_REMOTECALL_TYPE_IBV) {
        ...
        _ibvbdim[0] = _bdim.x; _ibvbdim[1] = _bdim.y; _ibvbdim[2] = _gdim.z;
#if !RPC_ONLY
        ibvDscudaLaunchKernelWrapper(dscudaLoadModule("./dscudatmp/reduceMultiGPU.cu.ptx", Ptxdata), 1, mangledname_,
                                 _ibvgdim, _ibvbdim, _smemsize, (RCstream)_stream,
                                 _narg, _ibvarg);
#endif
    }
        ...
        _gdimrc.x = _gdim.x; _gdimrc.y = _gdim.y; _gdimrc.z = _gdim.z;
        _bdimrc.x = _bdim.x; _bdimrc.y = _bdim.y; _bdimrc.z = _bdim.z;
        rpcDscudaLaunchKernelWrapper(dscudaLoadModule("./dscudatmp/reduceMultiGPU.cu.ptx", Ptxdata), 1, mangledname_,
                                 _gdimrc, _bdimrc, _smemsize, (RCstream)_stream,
                                 _rcargs);
    }
}
...
/*
 * stub for remote call to addVec.
 */
void
dscudaaddVec(dim3 _gdim, dim3 _bdim, size_t _smemsize, cudaStream_t _stream , int n0, int *data0, int n1, int *data1)
{
    int _narg = 4;
    int _ibvgdim[3], _ibvbdim[3];
    ...
    if (dscudaRemoteCallType() == RC_REMOTECALL_TYPE_IBV) {
        ...
        _ibvargp->type = dscudaArgTypeI;
        _ibvargp->offset = _off;
        _ibvargp->val.intval = n0;
        _ibvargp->size = sizeof(int);
        _off += _ibvargp->size;


        // a pointer to a device-address 'dscudaAdrOfUva(data0)'.
        _ibvargp = _ibvarg + _rcargc;
        _rcargc++;
        _devptr = (void*)(size_t)dscudaAdrOfUva(data0);
        ALIGN_UP(_off, __alignof(_devptr));
        _ibvargp->type = dscudaArgTypeP;
        _ibvargp->offset = _off;
        _ibvargp->val.pointerval = (RCadr)_devptr;
        ...
        _ibvargp->type = dscudaArgTypeI;
        _ibvargp->offset = _off;
        _ibvargp->val.intval = n1;
        _ibvargp->size = sizeof(int);
        _off += _ibvargp->size;


        // a pointer to a device-address 'dscudaAdrOfUva(data1)'.
        _ibvargp = _ibvarg + _rcargc;
        _rcargc++;
        _devptr = (void*)(size_t)dscudaAdrOfUva(data1);
        ALIGN_UP(_off, __alignof(_devptr));
        _ibvargp->type = dscudaArgTypeP;
        _ibvargp->offset = _off;
        _ibvargp->val.pointerval = (RCadr)_devptr;
        ...
        _ibvbdim[0] = _bdim.x; _ibvbdim[1] = _bdim.y; _ibvbdim[2] = _gdim.z;
#if !RPC_ONLY
        ibvDscudaLaunchKernelWrapper(dscudaLoadModule("./dscudatmp/reduceMultiGPU.cu.ptx", Ptxdata), 2, mangledname_,
                                 _ibvgdim, _ibvbdim, _smemsize, (RCstream)_stream,
                                 _narg, _ibvarg);
#endif
    }
        ...
        _rcargp->val.type = dscudaArgTypeI;
        _rcargp->offset = _off;
        _rcargp->val.RCargVal_u.valuei = n0;
        _rcargp->size = sizeof(int);
        _off += _rcargp->size;
        ...
        _rcargp->val.type = dscudaArgTypeI;
        _rcargp->offset = _off;
        _rcargp->val.RCargVal_u.valuei = n1;
        _rcargp->size = sizeof(int);
        _off += _rcargp->size;
        ...
        _gdimrc.x = _gdim.x; _gdimrc.y = _gdim.y; _gdimrc.z = _gdim.z;
        _bdimrc.x = _bdim.x; _bdimrc.y = _bdim.y; _bdimrc.z = _bdim.z;
        rpcDscudaLaunchKernelWrapper(dscudaLoadModule("./dscudatmp/reduceMultiGPU.cu.ptx", Ptxdata), 2, mangledname_,
                                 _gdimrc, _bdimrc, _smemsize, (RCstream)_stream,
                                 _rcargs);
    }
}
...
#pragma end reduceMultiGPU.cuh

#define PERF_MEASUREMENT_ONLY (0) 
...
reduceInEachDevice(int ndev, int *h_idata)
{
    ...
    for (idev = 0; idev < ndev; idev++) {
        ...
        dscudareduceVec3(grids, threads, smemsize, NULL, dev->nelem, dev->idata, dev->odata);
    }
}
...
reduceDevicePair(Device_t *dev0, Device_t *dev1)
{
    ...
    dscudaaddVec(grids, threads, smemsize, NULL, dev0->nblock, dev0->odata, dev1->nblock, dev0->tmpdata);    
}
...
reduceWithMultiGPU(int ndev_req, int nelem, int *h_idata)
{
    ...
    retrieveResultFromDevice(Device + 0, h_odata); 
