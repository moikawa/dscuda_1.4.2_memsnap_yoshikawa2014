...
static verbAllocatedMem *verbAllocatedMemQuery(void *dst) {
...
static void verbAllocatedMemRegister(void *dst, int size) {
...
static void verbAllocatedMemUnregister(void *dst) {
...
static void verbAllocatedMemUpdate(void *dst, void *src, int size) {
  verbAllocatedMem *mem = verbAllocatedMemQuery(dst);

  if (!mem) return;

  memcpy(mem->src, src, size);
}

void dscudaVerbRealloc(void) {
  verbAllocatedMem *mem = verbAllocatedMemListTop;
  int verb = dscudaGetAutoVerb();

  dscudaSetAutoVerb(0);
  while (mem) {
    cudaMalloc(&mem->dst, mem->size);
    cudaMemcpy(mem->dst, mem->src, mem->size, cudaMemcpyHostToDevice);
    mem = mem->next;
  }
  dscudaSetAutoVerb(verb);
}
...
void dscudaVerbAddHist(int funcID, void *argp) {
  ...
  if (DSCVMethodId != DSCVMethodMalloc && DSCVMethodId != DSCVMethodFree) {
  ...
  switch (funcID2DSCVMethod(funcID)) {
    case DSCVMethodMalloc: {
      cudaMallocArgs *args = (cudaMallocArgs *)argp;
      verbAllocatedMemRegister(args->devPtr, args->size);
      break;
    }

    case DSCVMethodFree: {
      cudaFreeArgs *args = (cudaFreeArgs *)argp;
      verbAllocatedMem *mem = verbAllocatedMemQuery(args->devPtr);
    ...
    case DSCVMethodMemcpyH2D: {
      cudaMemcpyArgs *args = (cudaMemcpyArgs *)argp;

      verbAllocatedMemUpdate(args->dst, args->src, args->count);
      break;
    }
    case DSCVMethodMemcpyD2D: {
      cudaMemcpyArgs *args = (cudaMemcpyArgs *)argp;
