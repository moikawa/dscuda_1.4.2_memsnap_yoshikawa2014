...
cudaMalloc(void **devAdrPtr, size_t size)
{
    ...
    *devAdrPtr = dscudaUvaOfAdr(adrs[0], Vdevid[vid]);
    ...
    if (autoVerb) {
      ...
      args.size = size;
      dscudaVerbAddHist(dscudaMallocId, (void *)&args);
    }
...
cudaMemcpyD2H(void *dst, const void *src, size_t count, Vdev_t *vdev, CLIENT **clnt)
{
    ...
    if (step == 0) {
      ...
      for (int i = 0; i < Ncand; i++)
        if (strcmp(sp->ip, svrCand[i].ip)) {
          WARN(2, "reconnecting %s to %s\n", sp->ip, svrCand[i].ip);
          strcpy(sp->ip, svrCand[i].ip);
          sp->cid = svrCand[i].cid;
          setupConnection(Vdevid[vdevidIndex()], sp);
          dscudaVerbRealloc();
          Ncand = 0;
          break;
        }
