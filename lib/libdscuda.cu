...
dscudaSearchServer(char *ips, int size) {
  ...
  addr.sin_port = htons(RC_SERVER_IP_PORT + 1);
  addr.sin_addr.s_addr = inet_addr("255.255.255.255");

  sendto(sock, "DSCUDA_SERARCHSERVER", 20, 0, (struct sockaddr *)&addr, sizeof(addr));
  ...
  svr.sin_port = htons(RC_DAEMON_IP_PORT - 1);
  ...
  while (1) {
    memcpy(&readfds, &fds0, sizeof(fd_set));
    n = select(rcvsock + 1, &readfds, NULL, NULL, &timeout);

    if (n == 0)
      break;

    if (FD_ISSET(rcvsock, &readfds)) {
      memset(rcvbuf, 0, 256);
      recvfrom(rcvsock, rcvbuf, 256, 0, (struct sockaddr *)&svr, &sin_size);

      if (strncmp(rcvbuf, "DSCUDA_SERVERRESPONSE", 21) == 0) {
        if (size > strlen(ips) + strlen(inet_ntoa(svr.sin_addr)) + 1) {
          strcat(ips, " ");
...
initEnv(void)
{
    ...
    if (!env) {
      dscudaSearchServer(buf, 1024 * RC_NVDEVMAX);
    }
